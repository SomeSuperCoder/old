...
def generate_address(public_key):
    ...
    return "0x" + bitcoin_address.decode()


def generate_serializable_address(target):
    # Step 1: Compute the SHA-256 hash of the public key
    hash_sha256 = hashlib.sha256(target.serialize().encode()).digest()
...
def get_hash(target):
    # vm = randomx.RandomX(target.serialize(False).encode(), full_mem=False, secure=True, large_pages=False)
...
def send_token(blockchain, user_private_key: ecdsa.SigningKey, to, amount, token_address="", only_after=None, miner_fee=0, message=""):
    ...
    if (amount + miner_fee if token_address == "" else 0) > get_token_balance(blockchain, user_private_key.get_verifying_key(), token_address):
        print("Here!")
    ...
    #                 print("Halving!")
    #                 current_reward = max(current_reward / 2, 1)
    outputs.append(Output(user_private_key.get_verifying_key(), to, amount, token_address, "send"))
    if token_address != "":
        for tk in blockchain.get_token_list():
            tk: NewToken = tk
            if tk.address == token_address:
                if tk.burn:
                    outputs.append(Output(user_private_key.get_verifying_key(), "", amount * tk.burn, token_address, "burn"))
                if tk.commission:
                    outputs.append(Output(user_private_key.get_verifying_key(), generate_address(tk.public_key), amount * tk.commission, token_address, "commission"))
    ...
    # outputs.append(Output(generate_address(user_private_key.get_verifying_key()), input_sum - (output_sum-miner_fee), token_address, type="return"))

    new_transaction = Transaction(public_key=user_private_key.get_verifying_key(), outputs=outputs, only_after=only_after, message=message)
...
#     # print([i.serialize() for i in inputs])
#
#     for out in blockchain.get_output_list():
#         # out = input_to_output(blockchain, _in)
#         print("output result")
#         print(out.serialize())
#         if not out.type in ["burn", "gas", "reward", "mint"]:
...
def create_special_transaction(blockchain, private_key: ecdsa.SigningKey, type: str, token=None, amount=0, token_address="", only_after=None, miner_fee=0, message=""):
    ...
    match type:
        ...
        case "mint":
            ...
            print(f"NOW THEY ARE: {the_outputs}")
        case "stop_mint":
            the_outputs.append(
                Output(
                    "",
    ...
    # check user balance
    sums = get_output_sums_from_output_list(blockchain, the_outputs, private_key.get_verifying_key())
...
def sort_outputs_by_output_token_address(blockchain, outputs):
    result = {}
    for out in outputs:
        print("Out:")
        print(out)
        ...
        if result.get(out.token_address) is None:
            result[out.token_address] = []
        result[out.token_address].append(out)
...
def get_fee_amount_by_output(blockchain, output):
    ...
    print(f"The output is: {output.serialize()}")
    block_id = get_block_id_by_transaction_address(blockchain, output.transaction_address)
